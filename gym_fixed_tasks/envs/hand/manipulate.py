...
from gym import utils, error, rotations
from gym_fixed_tasks.envs import hand_env
from gym.envs.utils import robot_get_obs
...
class ManipulateEnv(hand_env.HandEnv, utils.EzPickle):
    def __init__(
        ...
        assert self.target_rotation in ['ignore', 'fixed', 'fixed_easy', 'xyz', 'x', 'y', 'z', 'parallel']
    ...
    def _sample_goal(self):
        ...
        initial_qpos = self.sim.data.get_joint_qpos('target:joint').copy()
        initial_quat = initial_qpos[3:]
        assert initial_quat.shape == (4,)
        ...
        if self.target_rotation == 'z':
            ...
            axis = np.array([0., 0., 1.])
            offset_quat = quat_from_angle_and_axis(angle, axis)
            target_quat = rotations.quat_mul(initial_quat, offset_quat)
        elif self.target_rotation == 'x':
            angle = self.np_random.uniform(-np.pi, np.pi)
            axis = np.array([1., 0., 0.])
            offset_quat = quat_from_angle_and_axis(angle, axis)
            ...
            offset_quat = quat_from_angle_and_axis(angle, axis)
            target_quat = rotations.quat_mul(initial_quat, offset_quat)
        elif self.target_rotation == 'fixed_easy':
            angle = np.pi*0.33
            # angle = 0
            axis = np.array([1., 0., 0.])
            offset_quat = quat_from_angle_and_axis(angle, axis)
            ...
            current_qpos = self.sim.data.get_joint_qpos('target:joint')
            target_quat = current_qpos[3:]
        else:
            raise error.Error('Unknown target_rotation option "{}".'.format(self.target_rotation))
    ...
    def set_goal(self, new_goal):
        assert new_goal.shape == self.goal.shape
...
class HandBlockEnv(ManipulateEnv):
    def __init__(self, target_position='random', target_rotation='xyz', reward_type='sparse'):
        super(HandBlockEnv, self).__init__(
            ...
            randomize_initial_rotation=False,randomize_initial_position=False,
            ignore_z_target_rotation = False,
            reward_type=reward_type, distance_threshold=0.01, rotation_threshold=0.1)
...
class HandPenEnv(ManipulateEnv):
    def __init__(self, target_position='random', target_rotation='xyz', reward_type='sparse'):
        super(HandPenEnv, self).__init__(
            ...
            randomize_initial_rotation=False,randomize_initial_position=False,
            ignore_z_target_rotation = False,
            reward_type=reward_type, distance_threshold=0.05)
